...
**GET: api/v1/assignments?limit=`<int>`&offset=`<int>`&ordering=`-add_date/add-date`&favourites=`<true/false>`&ordering=`-share/share`&language=`<str>`&assingment_type=`<str>`** - список задач
