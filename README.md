...
1. Открыть локальную ветку develop и подтянуть в нее последние изменения репозитория с гитхаба  `git pull`
2. Создать локально дочернюю ветку от develop для разрабатываемой фичи (при этом необходимо находиться непосредственно в ветке develop), желательно с именованием feature/название_вашей_фичи
3. Перейти в созданную ветку
4. Реализовать функционал / задачу
5. После окончания работы, вы выполняете такие шаги:

- Переходите в ветку develop
- `git pull` - пуллите изменения, которые могли там оказаться за время выполнения вашей задачи
- Возвращаетесь в свою ветку с feature/
- `git rebase develop` - вносите в свою ветку изменения, которые появились в девелопе (ваш код останется)
- `git add file_name` - желательно добавлять по несколько взаимосвязанных файлов и сразу коммитить их командой
- `git commit -m “сообщение о реализации функции”`

Зачем это нужно?
Если делаете несколько задач в одной ветке или что-то отдельное, тогда будет легче отслеживать изменения в файлах при мерже
...
`git push`
6. Создать pull request на слияние добавленной вами ветки с веткой develop и добавить в pull request одного из разработчиков вашего направления, чтобы он мог осуществить ревью кода.
7. Когда ревью завершено и все комментарии ревьюера выполнены - закрыть pull request и смержить его в develop.
8. После того, как актуальные изменения загрузились в develop - удалить созданную вами ветку новой фичи, если никакие изменения больше не потребуются.

### Примечания для sheriff - разработчика
1. Создать pull request из ветки develop в master
2. Проверить отсутствие конфликтов. Если они есть - решить в веб-редакторе. Если какие то конфликты касаются не вашей разработки - обратиться к ответственному разработчику
